...
# Faiss
bin_file='/home/duy/Documents/Competition/Video-Text-Retrieval/Data4SubTask/DataFromFaiss4Database/faiss_cosine.bin'
json_path = 'dict/keyframes_id(sai).json'
...
##### ĐỌC 2 FILE info_ocr và info_asr
...
##### ĐỔI TÊN CÁC CỘT CỦA FILE info_asr THÀNH: video_id, frame_id, asr
...
def thumbnailimg():
    ...
    if LenDictPath-1 > index+imgperindex:
        ...
        while tmp_index < last_index:
            #age_filelist.append(DictImagePath[tmp_index])    
...
# 1 cái API train, Predict(service -  triển khai khách hàng), train process data
#  Predict -> triển khai khách hàng demo -> chạy riêng
# Proces data -> chạy riêng (collect data), xử lý nhiễu, missing value,.. => chạy riêng

# data -> json -> train -> model -> predict => user dùng
